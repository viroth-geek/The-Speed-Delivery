...
//            }
    ...
    public boolean insertByStreetString(AddressByStreetString address) {
        try {
//            if (!isAddressExist(address.getLongitude(), address.getLatitude())) {
            ContentValues values = getValueFromAddressByStreetString(address);
            return database.insert(AddressM.TABLE_ADDRESS, null, values) > 0;
//            }
        } catch (SQLException e) {
            LoggerHelper.showErrorLog(" SQL :" + e.getMessage());
...
//            }
    ...
    private ContentValues getValueFromAddressByStreetString(AddressByStreetString address) {
